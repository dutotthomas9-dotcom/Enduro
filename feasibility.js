...
function computeEvolvingFeasibility(profile, objective, recentFeedbacks, recentSessions) {
  ...
  // ── Signal 6 : écart temps cible vs capacité actuelle (règle 5 + 12) ───────
  // Un objectif très ambitieux augmente la pression sur chaque séance manquée.
  const paceEval = evaluatePaceVsTarget(profile, objective);
  if (paceEval) {
    if (paceEval.coherence === 'risky') {
      score -= 2;
      signals.push({ type: 'negative', text: `Objectif temps très ambitieux vs niveau actuel (écart ~${paceEval.gap_pct}%)` });
    } else if (paceEval.coherence === 'ambitious') {
      score -= 1;
      signals.push({ type: 'negative', text: `Objectif temps ambitieux — chaque séance qualité compte davantage` });
    }
    // coherent → pas de pénalité
  }
...
function buildMessage(state, signals, objective) {
  ...
  const timeCtx  = objective.target_time_sec
    ? ` en ${formatDuration(objective.target_time_sec)}`
    : '';

  const messages = {
    coherent: [
      `Tes dernières semaines sont solides. Sur cette trajectoire, ton objectif ${raceName}${timeCtx} reste bien calibré. Continue.`,
      `Tu suis le plan comme prévu. Les séances clés sont réalisées — l'objectif${timeCtx} est sur les rails.`,
      `Bonne dynamique. Les signaux sont positifs : régularité, ressenti maîtrisé, séances clés effectuées.`,
    ],
    ambitious: [
      `Ton objectif ${raceName}${timeCtx} reste possible, mais les dernières semaines montrent quelques signaux de charge. Les séances clés qui arrivent sont importantes.`,
      `L'objectif${timeCtx} est atteignable, mais la régularité va être décisive. Évite de manquer les séances longues et qualité.`,
      `Quelques semaines irrégulières — ça ne laisse plus beaucoup de marge pour ton objectif${timeCtx}. Concentre-toi sur les prochaines séances clés.`,
    ],
    risky: [
      `Le rythme des dernières semaines rend ton objectif${timeCtx} plus difficile à tenir. Ce n'est pas irrémédiable, mais si ça continue, on devrait réfléchir à un ajustement.`,
      `Ton corps envoie des signaux de fatigue. L'objectif${timeCtx} reste possible, mais la trajectoire actuelle est tendue.`,
      `Plusieurs séances importantes n'ont pas pu être réalisées. L'objectif${timeCtx} est sous pression — parle-moi si tu veux ajuster.`,
    ],
  };
