// ─────────────────────────────────────────────────────────────────────────────
// ENDURO — Moteur de génération V1.2
// Trois sports distincts : triathlon | course à pied route | trail
// Règles déterministes. Chaque décision est explicable.
// ─────────────────────────────────────────────────────────────────────────────
...
const PEAK_VOLUMES = {
  ...
  run_marathon:       540,
  run_trail_short:    360,
  run_trail_medium:   540,
  run_trail_long:     780,
  run_trail_ultra:   1080,
};
...
function buildSwimSession(type, durationMin, zones, date) {
  ...
  const templates = {
    endurance: {
      ...
      coach_intro: 'Séance de fond. Nager longtemps à allure confortable — pas se fatiguer. On ajoute un éducatif pendant que les bras sont encore frais.',
  ...
  return {
    ...
    duration_minutes: durationMin, distance_km: Math.round(durationMin * 0.045 * 10) / 10,
...
function buildBikeSession(type, durationMin, zones, date) {
  ...
  return {
    ...
    duration_minutes: durationMin, distance_km: Math.round(durationMin * 0.58 * 10) / 10,
...
function buildRunSession(type, durationMin, zones, date) {
  ...
  return {
    ...
    duration_minutes: durationMin, distance_km: Math.round(durationMin * 0.175 * 10) / 10,
...
function buildTrailSession(type, durationMin, zones, date, trailCategory) {
    ...
    endurance: {
      ...
      blocks: [
        { name: 'Sortie trail continue', duration_min: durationMin, zone_label: z('z2'),
          ...
          target_hr: zones.hasHr ? `${zones.bpm.z1[0]}–${zones.bpm.z2[1]} bpm` : null,
          target_pace: zones.runZ2 ? `~${zones.runZ2} (terrain plat)` : 'allure conversationnelle' },
      ],
    },
    long_run: {
  ...
  return {
    ...
    distance_km: Math.round(durationMin * 0.13 * 10) / 10,
...
function getWeekPattern(sport, phase) {
  ...
  if (sport === 'run') {
    const p = {
      base: [
        { day:0, discipline:'rest', type:'rest',      pct:0    },
        { day:1, discipline:'run',  type:'endurance', pct:0.18 },
        { day:2, discipline:'run',  type:'tempo',     pct:0.22 },
        { day:3, discipline:'rest', type:'rest',      pct:0    },
        { day:4, discipline:'run',  type:'endurance', pct:0.16 },
        { day:5, discipline:'run',  type:'long_run',  pct:0.30 },
        { day:6, discipline:'run',  type:'endurance', pct:0.14 },
      ],
      development: [
        { day:0, discipline:'rest', type:'rest',      pct:0    },
      ...
      specific: [
        ...
        { day:5, discipline:'run',  type:'long_run',  pct:0.34 },
        { day:6, discipline:'run',  type:'endurance', pct:0.18 },
      ],
      taper: [
        { day:0, discipline:'rest', type:'rest',      pct:0    },
        { day:1, discipline:'run',  type:'threshold', pct:0.22 },
        { day:2, discipline:'run',  type:'endurance', pct:0.20 },
        { day:3, discipline:'rest', type:'rest',      pct:0    },
        { day:4, discipline:'rest', type:'rest',      pct:0    },
        { day:5, discipline:'run',  type:'endurance', pct:0.30 },
        { day:6, discipline:'run',  type:'endurance', pct:0.28 },
      ],
    };
    return p[phase] || p.base;
  }
  ...
  if (sport === 'trail') {
    const p = {
      base: [
        ...
        { day:5, discipline:'trail', type:'long_run',  pct:0.34 },
      ...
      development: [
        ...
        { day:6, discipline:'trail', type:'endurance', pct:0.22 },
      ],
      specific: [
        { day:0, discipline:'rest',  type:'rest',      pct:0    },
        { day:1, discipline:'trail', type:'threshold', pct:0.16 },
      ...
      taper: [
        { day:0, discipline:'rest',  type:'rest',      pct:0    },
        { day:1, discipline:'trail', type:'threshold', pct:0.22 },
        { day:2, discipline:'trail', type:'endurance', pct:0.20 },
        { day:3, discipline:'rest',  type:'rest',      pct:0    },
        { day:4, discipline:'rest',  type:'rest',      pct:0    },
        { day:5, discipline:'trail', type:'endurance', pct:0.32 },
        { day:6, discipline:'trail', type:'endurance', pct:0.26 },
      ],
    };
    return p[phase] || p.base;
  }
...
function generatePlan(athlete, objective) {
  ...
  const peakVol   = Math.min(baseVol * 1.6, PEAK_VOLUMES[objective.discipline] || 480);
  ...
  for (let w = 0; w < totalWeeks; w++) {
    ...
    const sessions = pattern.map(slot => {
      ...
      if (slot.discipline === 'trail') return buildTrailSession(slot.type, durMin, zones, sessionDate, objective.discipline);
      if (slot.discipline === 'brick') {
        return buildBrickSession(Math.round(durMin * 0.60), Math.round(durMin * 0.40), zones, sessionDate);
      }
    }).filter(Boolean);
    ...
    weeks.push({
      ...
      target_volume_minutes: weekVol, sessions,
    });
  }
...
// ─── FAISABILITÉ — contextualisée par sport ───────────────────────────────────
...
function analyzeFeasibility(athlete, objective) {
  ...
  const hPerWeek = Math.round(athlete.weekly_hours_current || 6);
  ...
  if (volRatio < 0.30)      { score -= 35; issues.push('volume_trop_bas'); }
  else if (volRatio < 0.50) { score -= 18; issues.push('volume_bas'); }
  ...
  const msgs = {
    triathlon: {
      realistic: `${weeksAvailable} semaines depuis ton niveau actuel, c'est suffisant. Le plan sera exigeant mais réaliste si tu es régulier.`,
      ambitious: `${weeksAvailable} semaines avec ${hPerWeek}h/sem, c'est ambitieux. Pas impossible — mais ça ne laisse aucune marge.`,
      risky:     `${weeksAvailable} semaines avec ${hPerWeek}h/sem, c'est trop court pour cette distance triathlon sans risque de blessure.`,
    },
    run: {
      realistic: `${hPerWeek}h/sem et ${weeksAvailable} semaines — c'est bien calibré pour ton objectif. Le plan t'y amène progressivement.`,
      ambitious: `${hPerWeek}h/sem avec ${weeksAvailable} semaines, c'est faisable — mais chaque séance compte. La régularité est décisive.`,
      risky:     `${hPerWeek}h/sem en ${weeksAvailable} semaines, c'est un peu court pour cet objectif sereinement.`,
    },
    trail: {
      realistic: `${hPerWeek}h/sem et ${weeksAvailable} semaines — adapté à ton objectif trail. Le dénivelé est intégré progressivement.`,
      ambitious: `${hPerWeek}h/sem pour ce trail, c'est ambitieux. Le dénivelé demande plus que la distance seule.`,
      risky:     `${hPerWeek}h/sem avec ${weeksAvailable} semaines — insuffisant pour ce trail sans risque. Le dénivelé est exigeant pour les muscles.`,
    },
  };

  const sportMsgs = msgs[sport] || msgs.triathlon;
  if (score >= 75)      { verdict = 'realistic'; message = sportMsgs.realistic; suggestion = null; }
  else if (score >= 45) { verdict = 'ambitious'; message = sportMsgs.ambitious; suggestion = getAlternativeObjective(objective); }
  else                  { verdict = 'risky';     message = sportMsgs.risky;     suggestion = getAlternativeObjective(objective); }
