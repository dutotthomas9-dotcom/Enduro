...
function generatePlan(athlete, objective) {
  ...
  // Calculer le nombre de semaines en phase taper (pour getTaperSubPhase)
  const taperBlock = blocks.find(b => b.phase === 'taper');
  const taperTotalWeeks = taperBlock?.weeks || 1;
  const taperStartIndex = totalWeeks - taperTotalWeeks;

  const weeks = [];
  let rollingVol = baseVol;

  // Jours préférés de l'athlète (disponibilités)
  const preferredDays = Array.isArray(athlete.available_days)
    ? athlete.available_days.map(Number)
    : (typeof athlete.available_days === 'string' && athlete.available_days
        ? JSON.parse(athlete.available_days)
        : []);
  ...
  for (let w = 0; w < totalWeeks; w++) {
    ...
    // ── Sous-phase d'affûtage (s3/s2/s1) ─────────────────────────────────
    const taperSub = isTaper
      ? getTaperSubPhase(w - taperStartIndex, taperTotalWeeks)
      : null;

    // ── Volume hebdomadaire ───────────────────────────────────────────────
    let weekVol;
    if (isTaper && taperSub) {
      // V1.21.x : volume d'affûtage calculé depuis le pic réel
      weekVol = Math.round(peakVol * TAPER_VOLUME_MULTIPLIERS[taperSub]);
    } else if (isRecovery) {
      weekVol = Math.round(rollingVol * 0.65);
    } else {
      const variation = weekVolumeVariation(w, 'charge');
      weekVol = Math.min(Math.round(rollingVol * 1.08 * variation), peakVol);
      rollingVol = Math.min(rollingVol * 1.08, peakVol);
    }
    ...
    // ── Pattern : V1.21.x si disponible, sinon fallback existant ─────────
    const phaseInWeek = block.phase;
    const weekIndexInPhase = w - blocks.slice(0, blocks.indexOf(block)).reduce((s, b) => s + b.weeks, 0);

    const enrichedPattern = getWeekPatternV2(sport, phaseInWeek, weekIndexInPhase, taperSub);
    const pattern = enrichedPattern || getWeekPattern(sport, phaseInWeek);

    // ── Ajustement des jours préférés ─────────────────────────────────────
    const slotsByDay = {};
    ...
    if (preferredDays.length > 0 && sport !== 'triathlon') {
      const adjustedSlots = {};
      const usedDays = new Set();
      for (const [day, slots] of Object.entries(slotsByDay)) {
        const d = parseInt(day);
        if (slots.every(s => s.discipline === 'rest')) { adjustedSlots[d] = slots; continue; }
        if (preferredDays.includes(d)) { adjustedSlots[d] = slots; usedDays.add(d); }
        else {
          let moved = false;
          for (let delta = 1; delta <= 2; delta++) {
            for (const sign of [1, -1]) {
              const nd = ((d + sign * delta) + 7) % 7;
              if (preferredDays.includes(nd) && !usedDays.has(nd) && !adjustedSlots[nd]) {
                adjustedSlots[nd] = slots; usedDays.add(nd); moved = true; break;
              }
            }
            if (moved) break;
          }
          if (!moved) { adjustedSlots[d] = slots; usedDays.add(d); }
        }
      }
      Object.keys(slotsByDay).forEach(k => delete slotsByDay[k]);
      Object.assign(slotsByDay, adjustedSlots);
    }
    ...
    const sessions = [];
    const sortedDays = Object.keys(slotsByDay).map(Number).sort((a, b) => a - b);
    ...
    for (const day of sortedDays) {
      ...
      for (const slot of daySlots) {
        if (slot.discipline === 'rest') continue;

        const durMin = Math.max(20, Math.round(weekVol * slot.pct));

        // ── Essayer d'abord le catalogue V1.21.x ────────────────────────
        const catalogSession = buildSessionFromCatalog(slot.type, {
          durationMin: durMin,
          zones,
          date: sessionDate,
          taperSub,
          trailCategory: objective.discipline,
          targetDplus: objective.target_dplus || null,
        });
        ...
        if (slot.discipline === 'swim')  sessions.push(buildSwimSession(slot.type, durMin, zones, sessionDate));
          ...
          // Contrainte affûtage triathlon : brick ≤50 min en S-3, ≤30 min en S-2, interdit en S-1
          let brickDur = durMin;
          if (taperSub === 's3') brickDur = Math.min(durMin, 50);
          if (taperSub === 's2') brickDur = Math.min(durMin, 30);
          if (taperSub === 's1') { sessions.push(buildRestSession(sessionDate)); continue; }
          sessions.push(buildBrickSession(Math.round(brickDur * 0.60), Math.round(brickDur * 0.40), zones, sessionDate));
        }
      }
    }
    ...
    const actualVol  = trainingSessions.reduce((s, x) => s + (x.duration_minutes || 0), 0);
    const weekDplus  = trainingSessions.reduce((s, x) => s + (x.d_plus || 0), 0);
    const weekKm     = trainingSessions.reduce((s, x) => s + (x.distance_km || 0), 0);
    const weekDistM  = trainingSessions.reduce((s, x) => s + (x.distance_m || 0), 0);

    weeks.push({
      week_number: w + 1, phase: block.phase, phase_name: block.name,
      week_type: weekType,
      taper_sub: taperSub,  // s3/s2/s1 ou null — exposé au frontend
      start_date: weekStart.toISOString().split('T')[0],
      target_volume_minutes: actualVol,
      weekly_km:     Math.round(weekKm * 10) / 10,
      weekly_d_plus: weekDplus,
      weekly_dist_m: weekDistM,
      sessions,
    });
  }
