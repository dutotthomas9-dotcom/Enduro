...
function computeAdaptation(feedback, session, nextSessions) {
  // nextSessions : tableau des séances à venir dans la même semaine ou la semaine suivante

  // ── Cas 1 : séance non réalisée ──────────────────────────────────────────
  if (feedback.status === 'skipped') {
    // On cherche un créneau libre dans les 5 prochains jours
    const reschedule = nextSessions.find(s =>
      s.discipline === 'rest' &&
      daysBetween(session.date, s.date) <= 5
    );
    if (reschedule) {
      return {
        change_type: 'reschedule',
        target_session_id: reschedule.id,
        original_type: reschedule.session_type,
        new_type: session.session_type,
        original_duration: 0,
        new_duration: Math.round(session.duration_minutes * 0.85),
        change_description: `Séance déplacée au ${formatDate(reschedule.date)}`,
        change_reason: `Tu n'as pas pu faire ta séance ${disciplineLabel(session.discipline)} de ${formatDate(session.date)}. J'ai trouvé un créneau ${formatDate(reschedule.date)} — version légèrement raccourcie pour ne pas surcharger la semaine.`,
      };
    }
    // Pas de créneau : on note sans modifier
    return {
      change_type: 'none',
      change_description: 'Aucun ajustement nécessaire',
      change_reason: `Séance manquée enregistrée. Pas de créneau disponible cette semaine pour la récupérer — le plan continue normalement.`,
    };
  }
